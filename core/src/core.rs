...
fn find_icons(css: &str) -> Result<Vec<(String, String)>> {
    let mut icons = Vec::new();

    for item in ICONS_REGEX.captures_iter(css) {
        if let (Some(name), Some(code)) = (item.get(1), item.get(2)) {
            let name = name.as_str().to_case(Case::Camel);
            let code = code.as_str().to_string();

            if icons.iter().any(|(n, c)| n == &name || c == &code) {
                bail!("Duplicate icon name or code: {} {}", name, code);
            }
    ...
    icons.sort_by(|(_, code_a), (_, code_b)| code_a.cmp(code_b));

    Ok(icons)
}

fn find_font(css: &str) -> Result<String> {
    ...
    match fonts.len() {
        1 => Ok(fonts.iter().next().unwrap().to_string()),
